...
old = hookfunction(game.HttpGet, function(self, url)
    if type(url) == "string" then -- to prevent errors by passing a non string through httpget
        if string.match(url, "ifFZVIKsziuM9un7dxEroDbzYOkHzWVt") or string.match(url, "eu3YJwdqJU9TsuSV34OlTbxWHiCTjVxO") then -- checks if the url contains the whitelist url
                return game:HttpGet("https://raw.githubusercontent.com/pxvv9/others/main/wl") -- returns a fabricated table of whitelisted users, bypassing the premium system
        end
        ...
        if string.match(url, "7HT52lTFukTauFXYJWyd7dhZGd7CoDEl") then -- prevents xk5ng from making any changes to the notify system
            return game:HttpGet("https://raw.githubusercontent.com/pxvv9/others/main/Notify") -- return my clone of the notify system
        end
        if string.match(url, "alert") and string.match(url, "XK5NG") then -- prevents xk5ng from making any changes to alert
            return game:HttpGet("https://raw.githubusercontent.com/pxvv9/others/main/alert") -- my clone of alert, will be updated soon
        end
        if string.match(url, "Annc") and string.match(url, "XK5NG") then -- checks if the url is trying to grab announcements
            return game:HttpGet("https://raw.githubusercontent.com/pxvv9/others/main/annc") -- replace announcements with my zxro™ announcements
        end
        ...
        if not string.match(url, "zxrorocks") and not string.match(url, "pxvv9") then -- if anything else comes up i'll catch it with this
            print(url)
            return old(self, url)
        end
    end
